...
__urdlllocal ur_result_t UR_APICALL urKernelSetLaunchAttributeExp(
    ...
    void *
        pAttrValue ///< [out][optional] array of bytes holding the launch attribute data.
    ...
    auto pfnSetLaunchAttributeExp =
        context.urDdiTable.KernelExp.pfnSetLaunchAttributeExp;

    if (nullptr == pfnSetLaunchAttributeExp) {
        return UR_RESULT_ERROR_UNSUPPORTED_FEATURE;
    }

    ur_kernel_set_launch_attribute_exp_params_t params = {
        &launchAttr, &attrID, &attrSize, &pAttrValue};
    uint64_t instance =
        context.notify_begin(UR_FUNCTION_KERNEL_SET_LAUNCH_ATTRIBUTE_EXP,
                             "urKernelSetLaunchAttributeExp", &params);

    ur_result_t result =
        pfnSetLaunchAttributeExp(launchAttr, attrID, attrSize, pAttrValue);

    context.notify_end(UR_FUNCTION_KERNEL_SET_LAUNCH_ATTRIBUTE_EXP,
                       "urKernelSetLaunchAttributeExp", &params, &result,
                       instance);
...
__urdlllocal ur_result_t UR_APICALL urGetKernelExpProcAddrTable(
    ...
    dditable.pfnSetLaunchAttributeExp = pDdiTable->pfnSetLaunchAttributeExp;
    pDdiTable->pfnSetLaunchAttributeExp =
        ur_tracing_layer::urKernelSetLaunchAttributeExp;
